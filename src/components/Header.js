...
const Header = () => {
    ...
    return (
        <div className="header site-header h-[110px] py-[10px] w-full flex justify-center">
            {/* Desktop layout (>=769px) */}
            <div className="w-[1200px] mx-[32px] px-[15px] flex items-center justify-between site-header-inner site-header-desktop">
                {/* Logo */}
                <div className="block w-[300px] h-[90px] py-[10px] float-left px-[15px] cursor-pointer site-header-logo">
                    <Link to='/'>
                ...
                {/* right header */}
                <div className="float-right flex gap-[15px] site-header-right">
                    <div className="border border-solid border-[#e5e6ec] rounded-[40px] w-[350px] h-[45px] flex items-center site-header-search">
                    ...
                    <a className="flex items-center border-2  border-solid border-[#ff2d37] rounded-[45px] text-[#ff2d37] cursor-pointer hover:bg-[#ffc6c9] hover:text-white site-header-hotline" href="#">
                    ...
                    {userToken.accessToken ? (
                        <div className="relative h-[45px] w-[45px] bg-[#ff2d37] rounded-[50px] flex items-center justify-center text-white cursor-pointer group/user  " >
                            ...
                            <div className=" hidden w-[180px] bg-white p-[10px] text-center absolute z-50 top-[45px] border rounded-[10px] shadow-md group-hover/user:block ">
                                ...
                                <Link className="block bg-[#ff2d37] text-white border border-[#ff2d37] h-[35px] leading-[35px] rounded-[25px] mt-[5px] hover:bg-white hover:text-[#ff2d37]" to={`/order/user/${userData.user_id ?? userData._id}`}>Đơn mua</Link>
            ...
            {/* Mobile layout (<=768px) */}
            <div className="w-full px-[15px] site-header-mobile">
                <div className="flex items-center justify-between">
                    <div className="block w-[180px] h-[60px] cursor-pointer">
                        <Link to='/'>
                            <img src={logo} alt="logo" className='h-full' />
                        </Link>
                    </div>
                    ...
                    <div className="flex items-center gap-[10px]">
                        {userToken.accessToken ? (
                            <div className="relative h-[40px] w-[40px] bg-[#ff2d37] rounded-[50px] flex items-center justify-center text-white cursor-pointer group/user" >
                                {userImage ? (
                                    <img className='rounded-[500px] w-full h-full object-cover' src={userImage} alt="user-img"></img>
                                ) : (
                                    <BiSolidUser />
                                )}
                                <div className=" hidden w-[180px] bg-white p-[10px] text-center absolute z-50 top-[42px] right-0 border rounded-[10px] shadow-md group-hover/user:block ">
                                    <Link className="block bg-[#ff2d37] text-white border border-[#ff2d37] h-[35px] leading-[35px] rounded-[25px] mt-[5px] hover:bg-white hover:text-[#ff2d37]" to={'/account/user'}>Tài khoản</Link>
                                    <Link className="block bg-[#ff2d37] text-white border border-[#ff2d37] h-[35px] leading-[35px] rounded-[25px] mt-[5px] hover:bg-white hover:text-[#ff2d37]" to={`/order/user/${userData.user_id ?? userData._id}`}>Đơn mua</Link>
                                    <Link to='/' onClick={handleLogout} className="block bg-[#ff2d37] text-white border border-[#ff2d37] h-[35px] leading-[35px] rounded-[25px] mt-[5px] hover:bg-white hover:text-[#ff2d37]" >Đăng xuất</Link>
                                </div>
                        ...
                        <div className="relative h-[40px] w-[40px] bg-[#ff2d37] rounded-[50px] flex items-center justify-center text-white cursor-pointer group/cart" >
                            <Link to='/cart'>
                                <FaBasketShopping />
                            </Link>
                            <Cart />
                ...
                <div className="mt-[10px] border border-solid border-[#e5e6ec] rounded-[40px] h-[45px] flex items-center site-header-search">
                    <input className="inline-block w-full h-[40px] outline-none border-none m-[16px]" placeholder="Tìm kiếm..." value={inputValue} onChange={handleInput}></input>
                    <button className="inline-block text-[#ff2d37] font-bold text-[24px] w-[40px] h-[40px] bg-white border-none mx-[8px] rounded-full " onClick={handleSearch}>
                        <AiOutlineSearch />
                    </button>
                </div>
